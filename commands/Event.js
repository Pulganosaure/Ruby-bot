...
module.exports = {
  ...
  displayEventList: async function(message) {
  ...
  displayEventDetails: async function(message, args) {
    if(!args[1]) {
      message.reply("merci de précisé un Id valide")
      return 0
    }
    const Event = await getEventById(args[1])
    if(Event === "") {
      message.reply("aucun évenement ne correspond a cet id")
      return 0
    }
    const names = await GetNameList(Event.participants)
    renderEventEmbed(message, Event, names)
  },

  joinEventById: async function(message, details) {
    if(!details[1]) {
    ...
    if(Event) {
      const UserId = await getUserIdByDiscordUsername(message.author.username + "#" + message.author.discriminator)
      if(!UserId || UserId === "") {
        message.reply("Impossible de trouver votre profil, merci de vérifier que le pseudo discord est correct")
        return false
      }
      if(Event.participants.includes(UserId.userId)) {
        message.reply("Vous participez déjà à l'événement, utilisez !leave :eventId: pour vous désinscrire")
        return false
      }

      AddUserToEvent(details[1], UserId.userId)
      message.reply("votre participation a l'évenement \"" + Event.title + "\" a bien été enregistré")
    }
    else {
      message.reply("no event found for this Event")
    }
  },
  leaveEventById: async function(message, details) {
    if(!details[1]) {
      message.reply("Merci de précisé un id d'événement.")
      return false
    }
    ...
    if(Event) {
      ...
      if(!Event.participants.includes(UserId.userId)) {
        message.reply("Vous ne participez pas à cet événement, utilisez !join :eventId: pour vous inscrire")
        return false
      }
...
async function getFuturEvents(channel) {
  ...
  return results = await Event.find().where('startDate').gte(newDate)
}
