...
module.exports = {
  getGameData: async function(username, message) {
    ...
    if(!GameData)
    return true
    GetUsernameAndRank(GameData.participants)
    RespondMessage(message,GameInfoInArray(GameData.participants), getBanList(GameData.bannedChampions), username)
  },
  getSummonerProfil: async function(username, message) {
    const profil = await getAccountDataByUsername(username, message)
    if(!profil)
    return true
    const ranks = await axios.get(`https://euw1.api.riotgames.com/lol/league/v3/positions/by-summoner/${profil.id}?api_key=${ApiKey}`).then(res => res.data)
    ProfilRespond(message, profil, ranks)
  },
  getChampionDetails: async function(championName, message) {
    const champion = await axios.get(`http://ddragon.leagueoflegends.com/cdn/6.24.1/data/fr_FR/champion/${championName}.json`).then(res => res.data)
...
async function RespondMessage(message, gameData, ids, GivedplayerName)
{
  const ranks = await getRanksListById(ids)
  const embed = new Discord.RichEmbed()
  .setTitle("Game of " + GivedplayerName)
  .setColor(0x00AE86)
  .addField("Player :", "**"+gameData.player.join("\n")+"**", true)
...
function ProfilRespond(message, ProfilData, ranks) {
  ...
  .setColor(0x00AE86)
  .setThumbnail(`https://ddragon.leagueoflegends.com/cdn/7.12.1/img/profileicon/${ProfilData.profileIconId}.png`)
  for(let i =0 ; i < ranks.length; i++)
  {
    embed.addField(ranks[i].queueType, "**" +ranks[i].tier + " " + ranks[i].rank + "\t\t\"" + ranks[i].leagueName + "\"**")
  }
  message.reply({embed})
}
...
function ChampionRespond(message, Champion) {
  ...
  .setColor(0x00AE86)
  .setThumbnail(`http://ddragon.leagueoflegends.com/cdn/6.24.1/img/champion/${Champion.name}.png`)
...
function GameInfoInArray(players) {
  ...
  players.forEach(async function(player) {
    ...
    if(res.data) rank = getRankinSolo(res.data)
    GameInfo.player.push(player.summonerName)
